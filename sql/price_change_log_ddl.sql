...
CREATE TABLE IF NOT EXISTS dwd.disc_nio_dp_price_change_log (
    ...
    vendor_code                 STRING          COMMENT '合作伙伴编码(供应商)',
    ...
    price_void_date             STRING          COMMENT '价格消失日期(该价格从快照中消失的日期, NULL表示当前有效)',
    gps_create_time             STRING          COMMENT 'GPS价格创建时间(源系统中的创建时间)',
...
--
-- 4. BOM成本对比使用方式:
--    - 匹配键: SUBSTR(material_code,1,8) + 颜色 + factory_code + vendor_code
--    - 筛选条件: effect_month = 目标月份
--              AND price_create_date BETWEEN 上月25日 AND 本月25日
--    - 可通过 change_source_pricea/priceb 计算价格变动金额
--
-- 5. 建议追加字段(可选优化):
--    - material_price_amount / logistic_price_amount: 当前价格, 避免回关联源表
--    - price_source: 区分SOURCING_ADD等非正式价格
-- ===================================================================
